...
class Grid(object):
    ...
    def __init__(self,lat_rad,lon_rad,sphere_radius = None,gridtype='regular'):
        ...
        @param lon_rad: longitude [radians]
        @return:
        """
        self._lat0 = lat_rad
        self._lon0 = lon_rad
    ...
    def orthodrome(self,lon1,lat1,lon2,lat2):
        ...
        @return:
        """
        return np.arccos(np.sin(lat1)*np.sin(lat2)+np.cos(lat1)*np.cos(lat2)*np.cos(np.abs(lon2-lon1)))*self.radius
    ...
    def _calc_triangles(self,ax=None):
        ...
        for i in range(1,ny-1):   #todo revise this calculation method!!!
            for j in range(1,nx-1):
                ...
                #upper left
                lons.append(self._lon0[i-1,j-1]+0.5*(self._lon0[i,j]-self._lon0[i-1,j-1]))
                lats.append(self._lat0[i-1,j-1]+0.5*(self._lat0[i,j]-self._lat0[i-1,j-1]))

                #lower right
                lons.append(self._lon0[i,j]+0.5*(self._lon0[i+1,j+1]-self._lon0[i,j]))
                lats.append(self._lat0[i,j]+0.5*(self._lat0[i+1,j+1]-self._lat0[i,j]))

                #lower left
                #lons.append(self._lon0[i-1,j]+0.5*(self._lon0[i+1,j+1]-self._lon0[i,j]))



                #add also current center coordinate for plotting vizualization
    ...
    def plot_voronoi(self,ax=None,show_latlon=True):
        ...
        #calculate half size position of each edge and do delaunay interpolation again. This
        ...
        lons=[]; lats=[]
        for e in self.edg:
            plon = self.lon[e[0]] + 0.5*(self.lon[e[1]]-self.lon[e[0]]) #half way position
    ...
    def _do_delaunay_plot(self,tri,lon,lat,ax):
        ...
        Path = mpath.Path
        for t in tri:
            t_i = [t[0], t[1], t[2]]      # t[0], t[1], t[2] are the points indexes of the triangle
            verts = np.asarray([lon[t_i],lat[t_i]]).T #... vertices for a single triangle
        ...
        cmap = pl.cm.get_cmap('jet', 50); norm = mpl.colors.Normalize(vmin=None, vmax=None) #colorbar mapping
        self._collection = PatchCollection(patches, cmap=cmap,norm=norm, alpha=1.,match_original=False)  #construct library of all objects
        colors = 100*np.random.rand(len(patches))
        self._collection.set_array(np.array(colors)) #assign data values here
